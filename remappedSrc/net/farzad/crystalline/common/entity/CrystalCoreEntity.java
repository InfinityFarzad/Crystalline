...
public class CrystalCoreEntity extends Entity implements TraceableEntity {
    ...
    public void tick() {
        ...
        if (this.getOwner() instanceof Player player && crystalShootDelay >= 1) {
            for (int i = 4; i > 0; i--) {
                float pitch = -90 + Mth.randomBetween(player.getRandom(), -40.0f, 10.0f);
                float yaw = Mth.randomBetween(player.getRandom(), -360.0f, 360.0f);
                throwCrystal(player, pitch, yaw, this.level(), false, new Vec3(this.getX(), this.getY() + 1, this.getZ()));
    ...
    protected void readAdditionalSaveData(ValueInput view) {
        ...
        view.getBooleanOr("isRemovedFromExist",this.isRemoved);
    ...
    protected void addAdditionalSaveData(ValueOutput view) {
        ...
        view.putBoolean("isRemovedFromExist",this.isRemoved);
