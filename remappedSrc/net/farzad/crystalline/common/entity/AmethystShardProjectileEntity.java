...
public class AmethystShardProjectileEntity extends AbstractArrow {
    ...
    public void shootFromRotation(Entity shooter, float pitch, float yaw, float roll, float speed, float divergence) {
        ...
        this.setDeltaMovement(this.getDeltaMovement().add(vec3d.x, shooter.onGround() ? (double)0.0F : vec3d.y, vec3d.z));
    }
    ...
    private static DamageSource createDamageSource(Entity owner, ResourceKey<DamageType> damageType) {
        return new DamageSource(
                owner.level().registryAccess()
                        .lookupOrThrow(Registries.DAMAGE_TYPE)
                        .get(damageType.identifier()).get());
    }
