...
public class CrystallineClient implements ClientModInitializer {
    ...
    private void init() {
        ...
        SelectItemModelProperties.ID_MAPPER.put(Crystalline.id("divider_heart"), SelectItemModelProperty.Type.create(DividerHeartProperty.CODEC, Codec.STRING));
        initNetworking();
        CrystallineParticles.clientInit();
    }
    ...
    private void addTooltips() {
        ItemTooltipCallback.EVENT.register((itemStack, tooltipContext, tooltipType, list) -> {
            ...
            if (itemStack.is(CrystallineItems.DIVIDER)) {
                String dividerHeartTranslationPath;
                ItemStack dividerHeart = itemStack.getOrDefault(CrystallineDataComponentTypes.DIVIDER_HEART, ItemStack.EMPTY);
                String dividerHeartName = dividerHeart != ItemStack.EMPTY ? dividerHeart.getItemName().getString() : "Empty";

                list.add(1, Component.translatable(
                        "tooltip.crystalline.current_divider_heart",
                        Component.literal(dividerHeartName).withStyle(ChatFormatting.LIGHT_PURPLE)
                ).withStyle(ChatFormatting.GRAY));


                if (dividerHeart.is(CrystallineItems.CRYSTAL_CORE)) {
                    dividerHeartTranslationPath = "tooltip.crystalline.divider_shoot_cc";
                } else if (dividerHeart.is(CrystallineItems.CRYSTAL_HEART)) {
                    dividerHeartTranslationPath = "tooltip.crystalline.divider_shoot_ch";
                } else if (dividerHeart.is(Items.HEART_OF_THE_SEA)) {
                    dividerHeartTranslationPath = "tooltip.crystalline.divider_shoot_hots";
                } else {
                    dividerHeartTranslationPath = "tooltip.crystalline.divider_shoot";
                }
